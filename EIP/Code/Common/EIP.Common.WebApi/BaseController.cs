...
namespace EIP.Common.WebApi
{
    ...
    public class BaseController : Controller
    {
        ...
        /// <returns></returns>
        protected JsonResult JsonForGridPaging<T>(PagedResults<T> pagedResults, string msg = "")
        {
            return Json(new
            {
                code = 0,
        ...
        /// <returns></returns>
        protected JsonResult JsonForJsTree(IEnumerable<JsTreeEntity> datas, bool opened = true)
        {
            ...
            {
                //判断有无父级
                var ds = datas.Where(w => w.id.ToString() == data.parent.ToString());
                if (ds.Any() || data.parent.ToString() == Guid.Empty.ToString())
                {
