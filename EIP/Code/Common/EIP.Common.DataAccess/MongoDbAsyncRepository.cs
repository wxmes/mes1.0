...
namespace EIP.Common.DataAccess
{
    ...
    public class MongoDbAsyncRepository<T> : IAsyncMongoDbRepository<T> where T : class, new()
    {
        ...
        /// <returns></returns>
        public UpdateResult Update(T t, FilterDefinition<T> filter)
        {
            try
            {
                var client = MongodbInfoClient();
        ...
        /// <param name="filter"></param>
        /// <returns></returns>
        public async Task<UpdateResult> UpdateAsync(T t, FilterDefinition<T> filter)
        {
            try
            {
                var client = MongodbInfoClient();
        ...
        /// 根据id查询一条数据
        /// </summary>
        /// <param name="filter">objectid</param>
        /// <param name="field">要查询的字段，不写时查询全部</param>
        /// <returns></returns>
        public T FindOne(FilterDefinition<T> filter, string[] field = null)
        {
            try
            {
                var client = MongodbInfoClient();
