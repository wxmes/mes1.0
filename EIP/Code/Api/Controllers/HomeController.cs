...
using EIP.Common.WebApi.Extension;
...
namespace EIP.System.Api
{
    ...
    public class HomeController : BaseController
    {
            ...
            JsTreeEntity currTreeEntity)
        {
            ...
            {
                var remark = currTreeEntity.remark.IsNotNullOrEmpty()? "<small class='label pull-right bg-green'>" + currTreeEntity.remark + "</small>": "";
                childTrees.Append($@" <li>
                    ...
                    }'></i>{currTreeEntity.text} <span class='pull-right-container'>{remark}</span></a>
                        </li>");
            }
            else
            {
                childTrees.Append(string.Format(@"<li class='treeview'>
                            <a class='ajaxify' href='{0}' open-type='{3}'>
                                <i class='fa {1}'></i><span>{2}</span> 
                                <span class='pull-right-container'>
                                    <i class='fa fa-angle-left pull-right'></i>{4}
                                </span>
                            </a><ul class='treeview-menu'>"
                    , currTreeEntity.url.IsNullOrEmpty() ? "#" : currTreeEntity.url, currTreeEntity.icon.IsNullOrEmpty() ? "fa-tasks" : currTreeEntity.icon, currTreeEntity.text, "", currTreeEntity.remark.IsNotNullOrEmpty() ? "<small class='label pull-right bg-green'>" + currTreeEntity.remark + "</small>" : ""));
                //下面还有值
