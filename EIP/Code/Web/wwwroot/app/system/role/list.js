...
function roleUser() {
    ...
    GridIsSelectLayTable(tableObj, options.table.name, function (data) {
        DialogOpen("/Common/UserControl/ChosenPrivilegeMasterUser?" +
            ...
            "privilegeMasterValue=" + data.RoleId, "维护角色人员-" + data.Name, 800);
    }, true);
}
...
function copy() {
    ...
    GridIsSelectLayTable(tableObj, options.table.name, function (data) {
        var dialog = BootstrapDialog.show({
            ...
            title: '请输入角色名称-' + data.Name,
            message: $('<textarea id="copy-role-name" class="form-control" placeholder="请输入角色名称..." value="">' + data.Name + '</textarea>'),
            buttons: [{
                label: '确定',
                icon: 'fa fa-check',
                cssClass: 'eip-btn',
...
function menuPermission() {
    //查看是否选中
    GridIsSelectLayTable(tableObj, options.table.name, function (data) {
        DialogOpen("/System/Permission/Menu?privilegeMasterValue=" + data.RoleId + "&privilegeMaster=" + Language.privilegeMaster.role, "模块权限授权-" + data.Name, 380);
    }, true);
}
...
function functionPermission() {
    //查看是否选中
    GridIsSelectLayTable(tableObj, options.table.name, function (data) {
        DialogOpen("/System/Permission/Button?privilegeMasterValue=" + data.RoleId + "&privilegeMaster=" + Language.privilegeMaster.role, "模块按钮权限授权-" + data.Name, 810);
    }, true);
}
