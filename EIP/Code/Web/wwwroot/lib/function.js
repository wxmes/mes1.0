...
function UtilAjaxPostWithTypeWait(url, postData, type, success) {
    ...
    dialog.getModalBody().addClass("waitbody");
    dialog.getModalBody().css('background-size', '18px 18px');
    dialog.getModalBody().css('padding', '15px 10px 17px 15px');
...
function DataBaseGetFields(appId, table) {
    var fields = [];
    $.ajax({
        url: Language.common.apiurl + '/DataBase/GetDataBaseColumns',
        data: { id: appId, tableName: table },
        headers: {
            'Authorization': UtilGetAuthorizationLocalStorage()
        },
        dataType: "json",
 ...
 * 参数:connid:应用Id,table:表名,field:字段名
 */
function DataBaseBiddingFields(attJson, textid, $bindfiled) {
...
* 参数:$grid:列表对象,method选中后执行方法
*/
function GridIsSelectLayTable($grid, id, method, single) {
    var data = GridGetIdLayTable($grid, id).data;
    if (data.length == 0) {
        DialogTipsMsgWarn("请选择需要操作的数据", 1000);
    } else if (single) {
        if (data.length > 1) {
            DialogTipsMsgWarn("您选择了多条数据,请取消多余勾选！", 1000);
        } else {
            method(data[0]);
        }
    } else {
...
/**
* 作用:表格所有选中值Id,以逗号分割:如删除使用,多选,返回id
* 参数:$grid:列表对象,key:主键名称,method:执行方法,single:单选
*/
function GridIsSelectLayTableIds($grid, id,key, method, single) {
    //选中数据
    var data = GridGetIdLayTable($grid, id).data;
    if (data.length == 0) {
        DialogTipsMsgWarn("请选择需要操作的数据", 1000);
